...
class VpnStateManager {
  ...
  Duration get displayDuration => _finalConnectionDuration ?? connectionDuration.value;
  ...
  void _startConnectionTimer() {
    _connectionTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (vpnState.value == VpnEngine.vpnConnected && _connectionStartTime != null) {
        connectionDuration.value = DateTime.now().difference(_connectionStartTime!);
    ...
    Timer.periodic(const Duration(seconds: 1), (timer) {
      if (isConnecting.value && vpnState.value != VpnEngine.vpnConnected) {
        if (countdownSeconds.value > 0) {
          ...
          timer.cancel();
          print('⏰ Connection timeout after $_connectionTimeoutSeconds seconds');
