...
class VpnConnectionService {
  ...
  Future<void> _connectBasedOnProtocol(LocalVpnServer server, Vpn apiVpn) async {
  ...
  Future<void> _connectWireGuard(LocalVpnServer server) async {
    try {
      final config = await rootBundle.loadString('assets/wireguard/${server.configFileName}');
  ...
  Future<void> _connectStunnelWireGuard(LocalVpnServer server) async {
    try {
      ...
      final stunnelConfig = await rootBundle.loadString('assets/stunnel/${server.stunnelConfigFileName}');
      ...
      final wireguardConfig = await rootBundle.loadString('assets/stunnel/${server.configFileName}');
  ...
  void _handleConnectionError(String message, dynamic error) {
    ...
    if (_retryAttempts < _maxRetryAttempts) {
      _retryAttempts++;
      print('🔁 Connection error, will retry... ($_retryAttempts/$_maxRetryAttempts)');
    } else {
